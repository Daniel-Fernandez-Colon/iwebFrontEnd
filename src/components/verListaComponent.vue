...
export default {
    ...
    created() {
        ...
        if (this.$route.params.localidad) {
            ...
            axios.get("https://b1zw5s.deta.dev/appVivienda/viviendas/" + parametro).then(response => this.viviendas = response.data);
        } else {
            axios.get("https://b1zw5s.deta.dev/appVivienda/viviendas").then(response => this.viviendas = response.data);
        }
    },
    beforeUpdate() {
        // Simple GET request using axios
